...
$query = "SELECT R.name, R.comment, R.rating, R.price FROM Profiles P
...
if (mysql_num_rows($result) != 0) {
   ...
   while ($row = mysql_fetch_assoc($result)) {
      ...
      if (!is_null($comment) && !is_null($price) && !is_null($rating)) {
        ...
        array_push($comments, array('restaurant' => $restaurant, 'price' => $review,  'rating' => $rating));
      }
         ...
         array_push($comments, array('restaurant' => $restaurant, 'price' => $review));
      }
